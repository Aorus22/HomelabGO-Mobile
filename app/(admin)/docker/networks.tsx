...
export default function DockerNetworksScreen() {
    ...
    const fetchData = async () => {
        try {
            const res = await adminApi.listDockerNetworks();
            setData(res);
        } catch (error) {
            console.error(error);
        } finally {
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View className="p-8 items-center">
                        <Text color="tertiary">No networks found</Text>
                    </View>
                }
                renderItem={({ item }) => (
                    <View className="bg-card border border-border rounded-xl p-4 mb-3">
                        ...
                        {item.project_name && (
                            <Text variant="caption2" color="secondary">Project: {item.project_name}</Text>
                        )}
                    </View>
                )}
            />
